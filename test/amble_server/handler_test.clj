...
(deftest test-amble-server-api
  ...
  (testing "get players"
    (let [get-players-request  (mock-request/request
      ...
      (is (= ["player-one" "player-two" "player-three" "player-four"
  ...
  (testing "get player"
    (let [get-player-request  (mock-request/request
      ...
      (is (= [[0.5 0.8464] [0.525 0.8031] [0.475 0.8031] [0.55 0.7598]
              [0.5 0.7598] [0.45 0.7598] [0.575 0.7165] [0.525 0.7165]
              [0.475 0.7165] [0.425 0.7165]]
  ...
  (testing "get player (after made move)"
    (let [_ (main/handler
             (-> (mock-request/request
                  :post
                  (str "/game/" test-game-id "/player/player-one/move/0"))
      ...
      (is (= 200
             (:status get-player-response)))
      (is (= [[0.5 0.5] [0.525 0.8031] [0.475 0.8031] [0.55 0.7598] [0.5 0.7598]
              [0.45 0.7598] [0.575 0.7165] [0.525 0.7165] [0.475 0.7165]
              [0.425 0.7165]]
             (json/read-str (:body get-player-response)))))))
