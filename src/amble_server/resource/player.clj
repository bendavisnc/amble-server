  ...
  (first
   (for [game-db-find   [(game-db/find game-id)]
         ...
         :when          (or (= {player-db/success nil} player-db-find)
                            (do
                              (.info log
                                     "Cannot add player that already exists.")
                              (.info log
                                     (format "  \"%s\"  \n\"%s\\\""
                                             game-id
                                             player-id))
         ...
         (or (and (player-db/failure player-db-add)
                  (do
                    ...
                    (throw (new RuntimeException
                                (player-db/failure player-db-add)))))
             true)
         :when
         (or (and
              ...
              (do
                (.info
                 log
                 "The unexpected occurred while trying to add player to game.")
...
;;                 nil)
;;             ;;else
;;             (do (.info log "Player resource get all is being thrown from bad
;;             db result.")
;;                 (.info log (str "  "
